...
module.exports = (msg,args) => {
  return new Promise(async function (resolve, reject) {
    try {
      ...
      if (args.length < 1) {
        await msg.channel.send(embeds.genericSmall(errorMsgs.tooFewArguments.replace('<prefix>',msg.guild.appData.prefix)));
        return resolve();
      }
      ...
      if (field == 'footer')
        ...
        await msg.channel.send(embeds.genericSmall(errorMsgs.invalidSubcommand.replace('<prefix>',msg.guild.appData.prefix)));
        return resolve();
      }
