...
exports.askEmbed = (client,questionText,answerCount) => {
  const embed = new Discord.MessageEmbed()
    ...
    .setColor(config.embedColor)
    .setDescription('``' + questionText + '``\n' + 'This will cost you **' + answerCount * client.appData.settings.costPerAnswer + '** *favor* for **' + answerCount + '** *answer*' + ((answerCount == 1) ? '' : 's') + '. Please verify and agree to the [T&C](https://oraclix.com/termsAndConditions) by reacting with a 👍.')
    .setImage('')
    .setThumbnail('')
    //.setFooter(client.appData.settings.news)
  ;
...
exports.answerEmbed = (client,question,answerText) => {
  const embed = new Discord.MessageEmbed()
    ...
    .setColor(config.embedColor)
    .setDescription('``' + answerText + '``\n This will grant you **10** *favor*. Please verify and agree to the [T&C](https://oraclix.com/termsAndConditions) by reacting with a 👍.')
    .setImage('')
    .setThumbnail('')
    //.setFooter(client.appData.settings.news)
  ;
